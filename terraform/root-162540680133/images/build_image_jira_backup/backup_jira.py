...
class AtlassianBackup:
    ...
    def create_backup(self):
        url = requests.compat.urljoin(self.base_url, "/rest/backup/1/export/runbackup")
        ...
        url = requests.compat.urljoin(self.base_url, "/rest/backup/1/export/lastTaskId")
        task_request = self.session.get(url)
        task_id = task_request.text

        return task_id

    def wait_for_backup(self, task_id):
    ...
    def download_backup(self, backup_url: str, destination_dir: str):
        ...
        with self.session.get(
            requests.compat.urljoin(self.base_url, "/plugins/servlet/") + backup_url,
            stream=True,
        ) as r:
            r.raise_for_status()
...
def main():
    ...
    a = AtlassianBackup(
        ...
        base_url=JIRA_URL.format(args.account_id),
        user_name=args.user,
        token=args.token,
    )

    backup_task_id = a.create_backup()
    backup_url = a.wait_for_backup(task_id=backup_task_id)
